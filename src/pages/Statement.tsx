...
import { IndianRupee, ShoppingCart, CheckCircle, Clock, FileText, FileDown, FileSpreadsheet, Loader2, User, Share2, AlertCircle, Check, Lock, Key } from 'lucide-react';
...
const Statement: React.FC = () => {
  ...
  const [sheetApiUrl, setSheetApiUrl] = useLocalStorage('googleSheetApiUrl', 'https://sheetdb.io/api/v1/3ctrf6zyke3zi');
  ...
  const handleSyncToGoogleSheet = async () => {
    ...
    try {
        const dates = getDatesInRange(startDate, endDate);
        const syncPromises = [];
        const authConfig = (sheetApiUser && sheetApiPass) ? { auth: { username: sheetApiUser, password: sheetApiPass } } : {};
        ...
        for (const date of dates) {
            ...
            if (dataToSync.length > 0) {
                ...
                syncPromises.push(axios.patch(urlWithSheet, { data: dataToSync }, authConfig));
            }
        }
        ...
        if (failedSyncs.length === 0) {
            ...
            const firstError = failedSyncs[0] as PromiseRejectedResult;
            let errorMessage = 'An unexpected error occurred.';
            if (axios.isAxiosError(firstError.reason)) {
                if (firstError.reason.response?.status === 401) {
                    errorMessage = 'Error 401: Unauthorized. Please check your API Username and Password.';
                } else if (firstError.reason.response?.status === 405) {
                ...
                } else if (firstError.reason.response?.data?.error) {
                    errorMessage = `Sync failed: ${firstError.reason.response.data.error}`;
                } else {
                    errorMessage = `Sync failed: ${firstError.reason.message}`;
                }
            }
            setSyncStatus({ type: 'error', message: `Synced ${successfulSyncs} of ${results.length} days. ${failedSyncs.length} day(s) failed. First error: ${errorMessage}` });
        }
    } catch (error) {
      ...
      <div className="px-4">
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-4">
            Syncs a customer summary for each day in the selected date range. Each day will be a separate sheet. Requires a free account from a service like <a href="https://sheetdb.io" target="_blank" rel="noopener noreferrer" className="text-dairy-600 font-medium underline">SheetDB</a>.
          </p>
          <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg mb-4">
            <strong>Setup:</strong> Create a Google Sheet. The first row headers should be: <br/>
            ...
            <div className="mt-2 flex items-center">
              <Key size={14} className="mr-2 text-amber-600" />
              <div>
          ...
          <div className="space-y-4">
            ...
            <motion.button onClick={handleSyncToGoogleSheet} disabled={isSyncing || dataLoading} className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium flex justify-center items-center space-x-2 disabled:opacity-50" whileTap={{ scale: 0.98 }}>
